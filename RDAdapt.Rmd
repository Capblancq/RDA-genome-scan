...
## Genetic dataset and environmental variables loading
...
```{r, message=FALSE}
RDA <- rda(geno ~ env$envir1 + env$envir2 + env$envir3 + env$envir4 + env$envir5 + 
             env$envir6 + env$envir7 + env$envir8 + env$envir9 + env$envir10,  env)
```
...
ggplot() +
  ...
  geom_point(aes(x=c(1:length(RDA$CCA$eig)), y=as.vector(RDA$CCA$eig)), size = 3, 
             color="darkgrey") +
...
ggplot() +
  ...
  geom_point(aes(x=RDA$CCA$v[which(res_rdadapt[,2] < 0.1),1], 
                 y=RDA$CCA$v[which(res_rdadapt[,2] < 0.1),2]), col = "orange") +
  geom_segment(aes(xend=RDA$CCA$biplot[,1]/10, yend=RDA$CCA$biplot[,2]/10, x=0, y=0), 
               colour="black", size=0.5, linetype=1, 
               arrow=arrow(length = unit(0.02, "npc"))) +
  geom_text(aes(x=1.2*RDA$CCA$biplot[,1]/10, y=1.2*RDA$CCA$biplot[,2]/10, 
                label = colnames(env[,2:11]))) +
